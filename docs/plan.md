...
| 主視覺 | 首頁 Hero 的 `WorksChart`：座標網＋星座連線＋刻度，亮星＝作品（可點，直達段落） |
...
5. 兩欄 Hero 下標題級數要比通用 display 收一階（`clamp(2.2rem, 4.5vw, 3.3rem)`），
   否則 zh 標題會折行成孤字；文案結尾也別留破折號（zh 版已移除）。
...
```
/            首頁：Hero（宣言＋Hoshivel 星圖）→ α 作品 → β 新聞 → 理念一句 → γ 同行
/works       作品：每件完整段落（#shattered-realms / #hoshi-id）＋星表編號＋編號髮絲特點清單
