...
export type GlobalTheme = "auto" | "light" | "dark";
...
function writeCookie(name: string, value: string): void {
  ...
  if (domain) {
    // Remove the legacy host-only shape before writing the shared cookie.
...
export function readPreferredLanguage(): GlobalLanguage | null {
  if (typeof document === "undefined" || readConsent() !== CONSENT_PREFERENCES) return null;
  return normalizeGlobalLanguage(parseCookie(document.cookie, LANGUAGE_COOKIE));
}
...
export function rememberTheme(theme: GlobalTheme): void {
  writeCookie(CONSENT_COOKIE, CONSENT_PREFERENCES);
  if (theme === "auto") removeCookie(THEME_COOKIE);
  else writeCookie(THEME_COOKIE, theme);
}
